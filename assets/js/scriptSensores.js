...
async function fetchData(url) {
  try {
    let proxyUrl;
    if (url === "https://api-influx-fastapi.onrender.com/api/datos") {
    ...
    if (url === "https://api-influx-fastapi.onrender.com/api/datos") {
      ...
      if (!Array.isArray(data)) throw new Error("La API no retornó una lista.");
      return data;
    } else {
      // Respuesta de allorigins: { contents: "...json string..." }
...
function showMaxValues(data) {
  ...
  numericColumns.forEach((col) => {
    ...
    html += `<div style="background:#FFD700; padding:10px; border-radius:5px; flex:1;">
                    <strong>${col}</strong><br><span style="font-size:1.2em;">${maxVal.toFixed(
      2
    )}</span>
                </div>`;
  });
