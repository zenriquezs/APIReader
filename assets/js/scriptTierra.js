...
function showMaxValues(data) {
  ...
  const sample = data[0];
  numericColumns = Object.keys(sample).filter(
    (k) => typeof sample[k] === "number"
  );
  if (numericColumns.length === 0) {
    maxValuesDiv.innerHTML =
      "<p>No hay columnas numéricas para mostrar valores máximos.</p>";
    return;
  }
  let html =
    '<h3>Valores Máximos Detectados</h3><div style="display:flex; gap:20px;">';
  numericColumns.forEach((col) => {
    const maxVal = Math.max(
      ...data.map((row) => (row[col] != null ? row[col] : 0))
    );
    html += `<div style="background:#FFD700; padding:10px; border-radius:5px; flex:1;"><strong>${col}</strong><br><span style="font-size:1.2em;">${maxVal.toFixed(
      2
    )}</span></div>`;
  });
  html += "</div>";
...
function createFilters(data) {
  ...
  const sample = data[0];
  dropdownOptions = Object.keys(sample).filter(
  ...
  dropdownOptions.forEach((col) => {
    ...
    html += `<label><strong>${col}</strong></label><br><select multiple id="filter-${col}">`;
...
function applyFilters() {
  ...
  dropdownOptions.forEach((col) => {
    ...
    filteredData = filteredData.filter((row) =>
      selectedOptions.includes(row[col])
    );
  });
...
function showCharts(data) {
  ...
  Plotly.newPlot("chartContainer", [trace], {
    title: `Ejemplo: ${categoricalCols[0]} vs ${numericCols[0]}`,
  });
}
...
function populateCustomChartControls() {
  ...
  const xOptions = Object.keys(sample).filter(
    (k) => typeof sample[k] === "string"
  );
  const yOptions = Object.keys(sample).filter(
    (k) => typeof sample[k] === "number"
  );
...
document.getElementById("generateChartBtn").onclick = () => {
  ...
  Plotly.newPlot("customChartOutput", [trace], {
    title: `${chartType.toUpperCase()}: ${xVar} vs ${yVar}`,
  });
};
